...
struct APIClient {
    ...
    func health() async throws {
        guard let url = URL(string: baseURL.trimmingCharacters(in: CharacterSet(charactersIn: "/")) + "/healthz") else { throw APIError.invalidURL }
    ...
    func upload(_ batch: UploadBatch) async throws -> UploadResult {
        guard let url = URL(string: baseURL.trimmingCharacters(in: CharacterSet(charactersIn: "/")) + "/upload/v1/batches") else { throw APIError.invalidURL }
